...
checkSerieTemplate = 'checkSerie.html'
...
def home():
    ...
    if request.method == 'POST':
        ...
        if request.form.get('s1Button') == s1_value:
            ...
            return redirect(url_for("checkSerie"))
...
@app.route(f"/{database.getCurrentSerieName()}", methods=['POST', 'GET'])
def checkSerie():
    ...
    if request.method == 'POST':
        if request.form.get('loginButton') == 'loginButton':
            return redirect(url_for('login'))

        elif request.form.get('signUpButton') == 'signUpButton':
            return redirect(url_for('signUp'))
    ...
    return render_template(checkSerieTemplate, serie_title=serie_title, serie_image_background=serie_image_background, serie_cover_image=serie_cover_image,
